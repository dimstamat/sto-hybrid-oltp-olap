...
public:
    ...
    explicit db_profiler(bool spawn_perf)
            : spawn_perf_(spawn_perf), DB_size(0), perf_pid_(),
              start_tsc_(), end_tsc_() {}

    explicit db_profiler(bool spawn_perf, int db_sz)
            : spawn_perf_(spawn_perf), DB_size(db_sz), perf_pid_(),
              start_tsc_(), end_tsc_() {}
    ...
    void finish(size_t num_txns) {
        ...
        std::cout << "Throughput" << (DB_size>0? " (TPCH): ": ": ") << (double) num_txns / (elapsed_time / 1000.0) << (DB_size==0? " txns/sec" : (" q/sec @"+ std::to_string(DB_size/1024/1024) + "MB") ) << std::endl;
...
private:
    bool spawn_perf_;
    int DB_size; // 0 for tpcc, >0 for tpch
    pid_t perf_pid_;
    uint64_t start_tsc_;
    uint64_t end_tsc_;
};
